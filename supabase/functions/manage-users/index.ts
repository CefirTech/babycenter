...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "list") {
      ...
      const { data: rolesRows } = await admin.from("user_roles").select("user_id, role").in("user_id", safeIds);
      const { data: profilesRows } = await admin.from("profiles").select("user_id, avatar_url, display_name").in("user_id", safeIds);
      ...
      (profilesRows ?? []).forEach((p: any) => profileByUser.set(p.user_id, p));
    ...
    if (action === "create") {
      ...
      const { data: created, error } = await admin.auth.admin.createUser({
        email, password, email_confirm: true, user_metadata: { display_name, avatar_url },
      });
      if (error) throw error;
      const uid = created.user!.id;
      // Le trigger handle_new_user crée la ligne profiles ; on met à jour avatar_url + display_name
    ...
    if (action === "update") {
      ...
      const avatar_url = body.avatar_url !== undefined ? (body.avatar_url ? String(body.avatar_url) : null) : undefined;
